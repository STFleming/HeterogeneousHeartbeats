...
int64_t hrm_get_last_timestamp(heart_rate_monitor_t volatile *hb){
	return hb->log[hb->state->read_index].timestamp;
}

double hrm_get_hw_rate(heart_rate_monitor_t volatile * hb)
{
	double temp_hr;
	int temp_index = hb->state->read_index;
	int prev_index;
	if(temp_index == 0) {prev_index = 10;}
	else { prev_index = temp_index - 1; }

	temp_hr = 1.0 /((double)(hb->log[temp_index].timestamp) - (double)(hb->log[prev_index].timestamp));	   
	return temp_hr;
}
