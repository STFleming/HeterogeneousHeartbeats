...
flush_rate_counter = 0;
  ...
  int64_t phys_addr = get_physical_addr(pid, hb->log);
  //printf("Pages containing shared heartbeat record are now pinned in memory.\n");
  //printf("--------------------------------------------\nPhysical location of log := %llX \t\t %llu\n", phys_addr, phys_addr);
    
    //void * hhb_device = setup_hhbquery(); //got the address of the hhb module
    //SENDING LOG LOCATION TO HHB_QUERY MODULE
    //hhbquery_set_log(hhb_device, phys_addr);


  //XExample - Setup the hardware that was generated by vivadoHLS
  XExample testing_hls;
  ...
  phys_addr = get_physical_addr(pid, hb->state);
  //printf("Physical location of global state := %llX \t\t %llu\n--------------------------------------------\n", phys_addr, phys_addr); 
    ...
    //hhbquery_set_log(hhb_device, phys_addr);
    phys_addr_state = phys_addr;
    //Unlock the hhbquery module
    //hhbquery_unlock(hhb_device);
    XExample_SetHeartbeat_record_phys_addr(&testing_hls, phys_addr); //Send the physical address to the hardware module
