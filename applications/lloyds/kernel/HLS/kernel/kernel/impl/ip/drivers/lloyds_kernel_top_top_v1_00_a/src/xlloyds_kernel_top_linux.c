...
typedef struct {
    ...
    XLloyds_kernel_top_uio_map maps[ MAX_UIO_MAPS ];
} XLloyds_kernel_top_uio_info;

/***************** Variable Definitions **************************************/
static XLloyds_kernel_top_uio_info uio_info;
...
static int uio_info_read_name(XLloyds_kernel_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XLloyds_kernel_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XLloyds_kernel_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XLloyds_kernel_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XLloyds_kernel_top_Initialize(XLloyds_kernel_top *InstancePtr, const char* InstanceName) {
	XLloyds_kernel_top_uio_info *InfoPtr = &uio_info;
