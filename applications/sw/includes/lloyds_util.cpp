...
// tree adder
coord_type tree_adder(coord_type *input_array,const uint m)
{
    #pragma HLS inline
    ...
    for(uint j=0;j<ceil(log2(m));j++)
    {
        ...
        if (j<ceil(log2(m))-1) {
            ...
            {
                #pragma HLS unroll
                coord_type tmp1 = input_array[2*i];
                coord_type tmp2 = input_array[2*i+1];
                coord_type tmp3 = tmp1+tmp2;
                input_array[i] = tmp3;
        ...
        if (j== ceil(log2(m))-1) {
            coord_type tmp1 = input_array[0];
            coord_type tmp2 = input_array[1];
            coord_type tmp3 = tmp1+tmp2;
            input_array[0] = tmp3;
            //printf("[%d] = [%d]+[%d]\n",0,0,1);
            #pragma HLS resource variable=tmp3 core=AddSubnS
...
void compute_distance(data_type p1, data_type p2, coord_type *dist)
{
    ...
    for (uint d=0; d<D; d++) {
        ...
        coord_type tmp_ext = tmp;
        coord_type tmp_mul = fi_mul(tmp_ext,tmp_ext);
