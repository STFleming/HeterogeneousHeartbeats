...
flush_rate_counter = 0;
  ...
  //int64_t log_phys_addr = (int64_t)get_physical_addr(pid, hb->log);
  //int64_t state_phys_addr = (int64_t)get_physical_addr(pid, hb->state);
...
int64_t heartbeat( heartbeat_t* hb, int tag, int amount)
{
   ...
   clock_gettime( CLOCK_REALTIME, &time_info );
    ...
    if(hb->first_timestamp == -1) {
      ...
      double instant_heartrate = (1.0*amount) /(((double) (time - old_last_time))) * 1000000000.0;
